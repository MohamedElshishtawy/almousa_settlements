            ...
            <table class="table">
                ...
                @foreach($companies as $company)
                    <tr wire:key="company-{{ $company->id }}">
                        ...
                        <td>
                            <input type="text" class="form-control @error('delegate_names.{{ $company->id }}') is-invalid @enderror"
                                   wire:model.live="delegate_names.{{ $company->id }}"
                                   wire:keyup.debounce.240="editField({{ $company->id }}, 'delegate_name', $event.target.value)">
                            @error('delegate_names.{{ $company->id }}') <span class="text-danger">{{ $message }}</span> @enderror
                        </td>
                        <td>
                            <input type="text" class="form-control @error('delegate_ranks.{{ $company->id }}') is-invalid @enderror"
                                   wire:model.live="delegate_ranks.{{ $company->id }}"
                                   wire:keyup.debounce.240="editField({{ $company->id }}, 'delegate_rank', $event.target.value)">
                            @error('delegate_ranks.{{ $company->id }}') <span class="text-danger">{{ $message }}</span> @enderror
                        </td>
                        <td>
                            <input type="text" class="form-control @error('delegate_phones.{{ $company->id }}') is-invalid @enderror"
                                   wire:model.live="delegate_phones.{{ $company->id }}"
                                   wire:keyup.debounce.240="editField({{ $company->id }}, 'delegate_phone', $event.target.value)">
                            @error('delegate_phones.{{ $company->id }}') <span class="text-danger">{{ $message }}</span> @enderror
                        </td>
                        <td>
                ...
                <tr>
                    ...
                    <td></td>
                    <td>
                        <input type="text" wire:model="name" class="form-control @error('name') is-invalid @enderror" placeholder="أكتب هنا">
                        @error('name') <span class="text-danger">{{ $message }}</span> @enderror
                    </td>
                    <td>
                        <input type="text" wire:model="delegate_name" class="form-control @error('delegate_name') is-invalid @enderror" placeholder="أكتب هنا">
                        @error('delegate_name') <span class="text-danger">{{ $message }}</span> @enderror
                    </td>
                    <td>
                        <input type="text" wire:model="delegate_rank" class="form-control @error('delegate_rank') is-invalid @enderror" placeholder="أكتب هنا">
                        @error('delegate_rank') <span class="text-danger">{{ $message }}</span> @enderror
                    </td>
                    <td>
                        <input type="text" wire:model="delegate_phone" class="form-control @error('delegate_phone') is-invalid @enderror" placeholder="أكتب هنا">
