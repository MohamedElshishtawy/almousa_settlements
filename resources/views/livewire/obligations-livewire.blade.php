    ...
    <div class="report-details">
        ...
        <table class="table table-borderless">
            ...
            <tr>
                <th>اسم المقر</th>
                <td>
                    <select class="form-select" wire:model="selectedOfficeId">
                        <option value="">اختر المقر</option>
                        @foreach ($offices as $office)
                            <option value="{{ $office->id }}" @if ($office->id === $selectedOfficeId) selected @endif>
                                {{ $office->name }}
                            </option>
                        @endforeach
                    </select>
    ...
    <div class="products-details">
        ...
        @foreach ($bands as $id => $band)
            <section class="mt-3">
                ...
                <div class="mt-1">
                    <textarea wire:model="contents.{{ $id }}" class="form-control"
                              @if (!in_array($id, $selectedBands)) disabled @endif></textarea>
                </div>
            </section>
        @endforeach
    </div>
