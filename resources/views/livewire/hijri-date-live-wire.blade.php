    ...
    <div class="card-body table-responsive">
        ...
        <table class="table">
                    ...
                    <td>
                        <select class="form-select"
                            ...
                            @foreach(\App\Models\HijriDate::$hijryMonths as $month => $monthName)
                                <option
                                    value="{{$month}}" {{ $date->month == $month ? 'selected' : '' }}>{{$monthName}}</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
                    ...
                    <td>
                        <select class="form-select"
                            ...
                            @foreach(\App\Models\Day::$days as $weekDay)
                                <option
                                    value="{{$weekDay}}" {{ $date->weekday == $weekDay ? 'selected' : '' }}>{{$weekDay}}</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
            ...
            <tr>
                ...
                <td>
                    <input type="number" min="1" max="31" class="form-control @error('day') is-invalid @enderror"
                           wire:model="day">
                    @error('day')
                    <div class="invalid-feedback">{{ $message }}</div> @enderror
                </td>
