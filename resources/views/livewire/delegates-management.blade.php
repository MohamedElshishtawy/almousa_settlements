            ...
            <div class="card-body table-responsive">
                ...
                <table class="table">
                    ...
                    @forelse ($delegates as $delegate)
                        <tr>
                            <td>
                                <input type="number" min="0" wire:model="delegatesNumbers.{{ $delegate->id }}" wire:change="changeNumber({{ $delegate->id }}, $event.target.value)" class="form-control">
                                @error('delegatesNumbers.' . $delegate->id)
                                <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </td>
                            <td>
                                <input type="text" wire:model="delegatesNames.{{ $delegate->id }}" wire:change="changeName({{ $delegate->id }}, $event.target.value)" class="form-control">
                                @error('delegatesNames.' . $delegate->id)
                                <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </td>
                            <td>
                                <input type="text" wire:model="delegatesInstitutions.{{ $delegate->id }}" wire:change="changeInstitution({{ $delegate->id }}, $event.target.value)" class="form-control">
